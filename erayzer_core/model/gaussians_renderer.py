...
def render_opencv_cam_gsplat(
    ...
    intr = torch.zeros(fxfycxcy.size(0), 3, 3, device=fxfycxcy.device, dtype=fxfycxcy.dtype) # v x 3 x 3
    intr[:, 0, 0] = fxfycxcy[:, 0]
    intr[:, 1, 1] = fxfycxcy[:, 1]
    intr[:, 0, 2] = fxfycxcy[:, 2]
    intr[:, 1, 2] = fxfycxcy[:, 3]
    ...
    render_colors, _, _ = rasterization(means3D, rotations, scales, opacity.squeeze(), 
                                        ...
                                        render_mode=render_mode) # v x h x w x 3
    if render_mode != 'RGB':
        render_colors, render_depth = render_colors[...,:3], render_colors[...,3:]
    else:
        render_depth = None
    return {
